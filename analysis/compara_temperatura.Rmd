...
v <- bind_rows(df10, dfno) %>% 
  mutate(variableF = case_when(
    variable == "TemperaturaSueloSuperficieCubierto" ~ "Suelo Cubierto",
    variable == "TemperaturaSueloSuperficieDespejado" ~ "Suelo Superficie",
    variable == "TemperaturaSuelo30profundidad" ~ "Suelo z-30cm", 
    variable == "TemperaturaAire_Estacion" ~ "Aire", 
    variable == "TemperaturaAire45altura" ~ "Aire")) %>% 
...
v %>% ggplot(aes(x=time, y = value, colour = location)) + 
  geom_point() + geom_line() + 
  facet_wrap(~variableF, ncol=1, strip.position = "right", scales = "free_y") + 
  theme_bw() +
  ylab("Temperatura") + xlab("") + 
